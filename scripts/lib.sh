...
# Parse AGENT_ID_XX / AGENT_GITHUB_TOKEN_XX(_FILE) env vars and populate
# the caller-declared seen_agents, agent_ids, and agent_tokens arrays.
# Exits on any validation error (missing id, missing token, duplicate id).
# Usage: load_agent_slots <max_slots>
#   Caller must declare before calling:
