...
case "$mode" in
  once)
    ...
    ;;
  loop)
    log "WARNING: RUN_MODE=loop (Phase 1 in-container supervisor) is deprecated."
    log "         Use the host controller instead: bash scripts/controller.sh"
    log "         See README Host Controller section for migration details."
